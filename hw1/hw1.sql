...
--6. Write a query that generates the following result set from Production.Product: NAME: LL Crankarm  --  COLOR: Black
...
SELECT DISTINCT ISNULL(p.ProductSubcategoryID, 0), ISNULL(p.Color, 'N/A')
FROM Production.Product p
WHERE p.ProductSubcategoryID != 0 AND p.Color != 'N/A'
